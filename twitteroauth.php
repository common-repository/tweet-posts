 ...
 *
 * Modifications for WordPress made by Josh Betz - http://joshbetz.com
 */
...
class TwitterOAuth {
	...
	function getRequestToken($oauth_callback = NULL) {
		...
		$request = $this->oAuthRequest(self::REQUEST_TOKEN, 'GET', $parameters);
	...
	function getAuthorizeURL($token, $sign_in_with_twitter = TRUE) {
		...
		if (empty($sign_in_with_twitter)) {
			return self::AUTHORIZE . "?oauth_token={$token}";
		} else {
		 return self::AUTHENTICATE . "?oauth_token={$token}";
	 }
 }
	...
	function getAccessToken($oauth_verifier = FALSE) {
		...
		$request = $this->oAuthRequest(self::ACCESS_TOKEN, 'GET', $parameters);
		$token = OAuthUtil::parse_parameters($request);
		$this->token = new OAuthConsumer($token['oauth_token'], $token['oauth_token_secret']);
		return $token;
	}
	...
	function getXAuthToken($username, $password) {
		...
		$request = $this->oAuthRequest(self::ACCESS_TOKEN, 'POST', $parameters);
		$token = OAuthUtil::parse_parameters($request);
		$this->token = new OAuthConsumer($token['oauth_token'], $token['oauth_token_secret']);
		return $token;
	}
